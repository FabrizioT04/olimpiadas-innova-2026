...
1. En el proyecto Apps Script de **PUNTAJES**, el que contiene `ArbitrajeSeguro.gs` y está vinculado a la hoja `Sábana`:
   - Sustituir el contenido de `ArbitrajeSeguro.gs` por la versión de esta propuesta. Conservar el `doGet` y sus auxiliares existentes.
   - Añadir `ResultadoUnificado.gs` como archivo de script en ese mismo proyecto.
...
2. En el proyecto Apps Script del **FIXTURE**:
   - Sustituir `Marcadores.gs` por la versión de esta propuesta. Conservar `FixtureOficial.gs`.
